...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Stream<AuthState> mapEventToState(AuthEvent event) async* {
    if (event is PhoneNumberVerificationEvent) {
      ...
      yield LoggedInState(user?.uid, user?.firstname, user?.lastname,
          user?.isActive, user?.latlng);
  ...
  Stream<AuthState> _setUpAccount(SignUpEvent event) async* {
    yield LoadingAuthState();
    final user = await UserRepository.instance!.setUpAccount(event.uid,
        event.firstname, event.lastname, event.isActive, event.latLng);
    yield LoggedInState(user?.uid, user?.firstname, user?.lastname,
        user?.isActive, user?.latlng);
  }
}
